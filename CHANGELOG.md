...
- **Small controls are easier to hit.** Seven tappable controls — the overflow
  menus on trigger, skill and file rows, the pin toggle on a memory entry, the
  preview button in the prompt-preset picker, the load/save buttons in the node
  editor, and the pipeline editor's toolbar buttons — had touch targets smaller
  than Android's 48 dp minimum, some as small as 28 dp. They looked right and
  missed often. All seven now reserve the full target; the affected rows are a
  few points taller as a result.
