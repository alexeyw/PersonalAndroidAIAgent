...
- `buildSrc/` - Build-logic module of pure-Kotlin, unit-tested build tools (compiled with Gradle's embedded Kotlin). Hosts `BrowserEditorConstantsGenerator` (regenerates the `pipeline-editor.html` `AUTO-GEN` blocks from the domain sources) and `DocsHygieneChecker` (scans the public docs for LLM tool-call artifacts and internal-document references). Both drive Gradle verification tasks wired into `check` (`verifyBrowserEditorConstants`, `verifyDocsHygiene`).
