 ...
 *
 * [CancelledBySystem] is intentionally distinct from [Failure]: a run the OS
 * cancelled (WorkManager stopped the worker under system pressure) is a platform
 * outcome, not a product defect, and conflating the two would hide exactly the
 * background-reliability signal the journal exists to expose.
 *
...
sealed interface TriggerRunOutcome {
    ...
    /**
     * The system cancelled the run (e.g. WorkManager stopped the worker under
     * resource pressure) before it reached a terminal state of its own.
     */
