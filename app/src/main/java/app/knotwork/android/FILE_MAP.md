...
- `domain/` - Domain layer containing core business logic and Use Cases.
  ...
  - `services/` - Domain-level services.
    ...
    - `HttpRequestPolicy.kt` - Pure, dependency-free security policy for the `http_request` tool: method→`ToolRisk` mapping (`GET` SENSITIVE / writes DESTRUCTIVE), domain-allowlist membership (exact + sub-domain), allowlist-entry normalisation/validation, https-vs-cleartext admissibility (loopback / RFC-1918 exception), and stored-credential leak detection. The single source both `HttpRequestExecutor` (enforcement) and `ToolRepositoryImpl.getRisk` (HITL strength) read, so the gate and the refusal cannot diverge.
