...
- `domain/` - Domain layer containing core business logic and Use Cases.
  ...
  - `usecases/` - Business logic Use Cases.
    ...
    - `LoadModelUseCase.kt` - Use case to load a model. Accepts `requireVision`: an image-carrying run forces a vision-enabling re-initialization of the LiteRT engine (re-uses an already vision-enabled engine for the same model; never forces a vision backend onto a freshly loaded text-only model).
    - `ResolveEntryInferenceUseCase.kt` - Classifies the inference entry of the pipeline a chat session would run (`EntryInferenceKind` = `LOCAL` / `CLOUD` / `NONE`) by resolving the bound-or-default graph and reading the `INPUT` node's successor. Backs the multimodal send-time pre-flight (a `CLOUD` entry blocks an image; otherwise the active model's vision flag decides).
