...
- `domain/` - Domain layer containing core business logic and Use Cases.
  ...
  - `usecases/` - Business logic Use Cases.
    ...
    - `MemoryImportUseCase.kt` - Imports a memory export file: `parse` surfaces the `MemoryImportOutcome`; `import(document, strategy, activeProviderId)` reconciles under `Merge` / `Replace`, preserving each chunk's id / provenance / pin state / tags and flagging imported chunks for re-embedding when the file's embedding provider differs (Phase 25 / Task 8).
    - `RecomputePendingEmbeddingsUseCase.kt` - Lazily re-embeds chunks imported under a different provider (`needsReembedding = 1`), batch-embedding their text with the active provider and clearing the flag. Invoked by `RetrieveRelevantMemoryUseCase` before each search (cheap `COUNT` no-op when nothing is pending) (Phase 25 / Task 8).
