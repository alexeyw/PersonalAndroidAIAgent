 ...
 *
 * Bundled presets live in `assets/presets/pipelines/*.json` and never
 * reach this table — only presets created via
 * `SavePipelineAsPresetUseCase` are persisted here. Storing the graph as a
 * JSON blob keeps preset rows self-contained (no FK explosion into
 * `pipeline_nodes` / `pipeline_connections`), which simplifies migration
 * when the pipeline schema evolves: the existing
 * `PipelinePresetJsonSerializer` already knows how to round-trip the
 * stored JSON through whatever version of the pipeline schema is current.
 *
