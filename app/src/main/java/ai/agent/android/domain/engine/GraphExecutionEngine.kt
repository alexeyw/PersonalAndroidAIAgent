...
class GraphExecutionEngine @Inject constructor(
    ...
    operator fun invoke(
        ...
        while (currentNode != null && stepCount < MAX_STEPS) {
            ...
            val nextNodeId = findNextNodeId(currentNode, graph, nodeResult?.conditionResult)
            currentNode = graph.nodes.find { it.id == nextNodeId }
        }
    ...
    private fun findNextNodeId(currentNode: NodeModel, graph: PipelineGraph, conditionResult: Boolean?): String? {
