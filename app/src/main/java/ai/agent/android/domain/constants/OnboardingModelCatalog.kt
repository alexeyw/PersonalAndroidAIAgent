 ...
 * ([ai.agent.android.presentation.ui.onboarding.OnboardingViewModel]):
 *  - to look up the expected `fileName` so it can call
 *    [ai.agent.android.domain.repositories.LocalModelRepository.isInstalled]
 *    when the user re-enters onboarding after a previous install;
 *  - to start the download via
 *    [ai.agent.android.domain.repositories.ModelDownloadManager.downloadModel]
...
object OnboardingModelCatalog {
    ...
    fun fileNameForCustomUrl(url: String): String {
        ...
        val lastSegment = trimmed.substringAfterLast(delimiter = '/', missingDelimiterValue = "")
